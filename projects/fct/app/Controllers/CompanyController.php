...
class CompanyController extends BaseController
{
    ...
    public function addCompanyAction()
    {
        ...
        if (isset($_POST['btn_create_company'])) {
            ...
            //Data validation
            if ((!empty($_POST['c_name'])) && (isset($_POST['c_name']))) {
                $company->setName(clearData($_POST['c_name']));

                if ((!empty($_POST['c_phone'])) && (isset($_POST['c_phone']))) {
                    $company->setPhone(clearData($_POST['c_phone']));

                    if ((!empty($_POST['c_email'])) && (isset($_POST['c_email']))) {
                        $company->setEmail(clearData($_POST['c_email']));

                        if ((!empty($_POST['c_address'])) && (isset($_POST['c_address']))) {
                            $company->setAddress(clearData($_POST['c_address']));
                        } else {
                            echo '<script>alert("La dirección de la empresa no puede estar vacía")</script>';
                        }
                    } else {
                        echo '<script>alert("El email de la empresa no puede estar vacío")</script>';
                    }
                } else {
                    echo '<script>alert("El teléfono de la empresa no puede estar vacío")</script>';
                }
            } else {
                echo '<script>alert("El nombre de la empresa no puede estar vacío")</script>';
            }
    ...
    //         echo 'hola';
    //         echo $company->getByCif();

    //         // if ($company->getByCif() != null) {
    //         //     echo '<script type="text/javascript">
    //         //         alert("La empresa con CIF ' . $_POST[clearData('c_cif')] . ' ya existe");
    //         //         </script>';
    //         // } else {
    ...
    //         //         var_dump($_POST['employeesList']);
    //         //         // $employeeList = json_decode($_POST['employeesList']);
    //         //         // $employee = Employee::getInstancia();

    //         //         // foreach ($employeeList as $value) {
    //         //         //     $employee->setName($value->name);
    //         //         //     $employee->setNif($value->nif);
    //         //         //     $employee->setJob($value->job);
